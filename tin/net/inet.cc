...
int INetNToP4(const unsigned char* src, char* dst, size_t size) {
 ...
 l = absl::SNPrintF(tmp, sizeof(tmp), fmt, src[0], src[1], src[2], src[3]);
...
static int INetNToP6(const unsigned char* src, char* dst, size_t size) {
  ...
  unsigned int words[sizeof(Inet6Addr) / sizeof(uint16_t)];
  int i;
  ...
  cur.base = -1;
  cur.len = 0;
  for (i = 0; i < (int) ABSL_ARRAYSIZE(words); i++) {    // NOLINT
    if (words[i] == 0) {
      if (cur.base == -1)
   ...
   */
  tp = tmp;
  for (i = 0; i < static_cast<int>(ABSL_ARRAYSIZE(words)); i++) {
  ...
  if (best.base != -1 && (best.base + best.len) == ABSL_ARRAYSIZE(words))
    *tp++ = ':';
  *tp++ = '\0';
